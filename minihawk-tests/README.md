...
The quickerNES regression suite (`quicknes/core/tests/*.test` + `.sol` input
sequences) must pass at every phase boundary, at two levels:
...
```
# one-time setup (WSL): copy quicknes/core + ROMs, then
...
`run-level-b.ps1` drives the actual EmuHawk build in `output/`: for each test it
...
ROMs are resolved from `quicknes/core/tests/roms/` first, then
